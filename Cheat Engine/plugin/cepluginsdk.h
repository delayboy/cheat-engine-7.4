...
typedef BOOL(__stdcall* CEP_DEBUG_SETBREAKPOINT)(UINT_PTR address, int size, int trigger, int method);
...
typedef BOOL(__stdcall** CEP_WritePROCESSMEMORY)(HANDLE hProcess, LPCVOID lpBaseAddress, LPVOID lpBuffer, SIZE_T nSize, SIZE_T* lpNumberOfBytesRead);
...
typedef HANDLE(__stdcall** CEP_OpenThread)(DWORD dwDesiredAccess, BOOL bInheritHandle, DWORD dwThreadId);
typedef DWORD(__stdcall** CEP_SuspendThread)(HANDLE hThread);

typedef BOOL(__stdcall* CE_GetThreadContext)(HANDLE hThread, LPCONTEXT lpContext);
typedef BOOL(__stdcall* CE_SetThreadContext)(HANDLE hThread, CONST CONTEXT* lpContext);
typedef CE_SetThreadContext* CEP_SetThreadContext;
typedef CE_GetThreadContext* CEP_GetThreadContext;

typedef DWORD(__stdcall** CEP_ResumeThread)(HANDLE hThread);
typedef BOOL(__stdcall** CEP_DebugActiveProcess)(DWORD dwProcessId);
typedef BOOL(__stdcall** CEP_WaitForDebugEvent)(LPDEBUG_EVENT lpDebugEvent, _In_ DWORD dwMilliseconds);
typedef BOOL(__stdcall** CEP_ContinueDebugEvent)(DWORD dwProcessId, DWORD dwThreadId, DWORD dwContinueStatus);
typedef BOOL(__stdcall** CEP_ChangeRegOnBP)(ULONG_PTR address,BOOL remove);
/*
...
typedef struct _ExportedFunctions
{
	...
	CEP_WritePROCESSMEMORY WriteProcessMemory;			//pointer to the pointer of WriteProcessMemory (Change it to hook that api, or use it yourself)
	CEP_GetThreadContext GetThreadContext;			//   ...
	CEP_SetThreadContext SetThreadContext;			//   ...
	CEP_SuspendThread SuspendThread;				//   ...
	CEP_ResumeThread ResumeThread;				//   ...
	PVOID OpenProcess;				//   ...
	CEP_WaitForDebugEvent WaitForDebugEvent;			//   ...
	CEP_ContinueDebugEvent ContinueDebugEvent;			//   ...
	CEP_DebugActiveProcess DebugActiveProcess;			//   ...
	PVOID StopDebugging;				//   ...
	...
	PVOID DebugProcess;				//   ...
	CEP_ChangeRegOnBP ChangeRegOnBP;				//   ...
